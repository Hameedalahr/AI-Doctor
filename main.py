...
def home():
    if request.method == 'POST':
        ...
        # 2️⃣ Invalid symptom check
        invalid_symptoms = [sym for sym in user_symptoms if sym not in symptoms_dict]

        if invalid_symptoms:
            message = f"⚠️ These symptoms are not recognized: {', '.join(invalid_symptoms)}"
            message += "<br>Please check spelling or try different symptoms."
            return render_template('index.html', message=message)

        # 3️⃣ Safe model prediction
        try:
            ...
            message = f"⚠️ Something went wrong: {str(e)}"
            return render_template('index.html', message=message)
